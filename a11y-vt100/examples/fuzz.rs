...
fn check_full(vt_base: &a11y_vt100::Screen, idx: usize) {
    let mut input = vec![];
    input.extend(vt_base.state_formatted());
    let mut vt_full = a11y_vt100::Parser::default();
...
fn check_diff_empty(
    vt_base: &a11y_vt100::Screen,
    empty: &a11y_vt100::Screen,
    idx: usize,
) {
    let mut input = vec![];
    input.extend(vt_base.state_diff(empty));
    let mut vt_diff_empty = a11y_vt100::Parser::default();
...
fn check_diff(
    vt_base: &a11y_vt100::Screen,
    vt_diff: &mut a11y_vt100::Parser,
    prev: &a11y_vt100::Screen,
    idx: usize,
) {
    let mut input = vec![];
...
fn check_rows(vt_base: &a11y_vt100::Screen, idx: usize) {
    let mut input = vec![];
    ...
    let mut vt_rows = a11y_vt100::Parser::default();
...
fn main() {
    let mut vt_base = a11y_vt100::Parser::default();
    let mut vt_diff = a11y_vt100::Parser::default();

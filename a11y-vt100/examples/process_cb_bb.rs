...
struct Callbacks;
impl a11y_vt100::Callbacks for Callbacks {
    fn audible_bell(&mut self, screen: &mut a11y_vt100::Screen) {
        std::hint::black_box(screen);
    }

    fn visual_bell(&mut self, screen: &mut a11y_vt100::Screen) {
        std::hint::black_box(screen);
    }

    fn resize(&mut self, screen: &mut a11y_vt100::Screen, request: (u16, u16)) {
        std::hint::black_box((screen, request));
    }

    fn set_window_icon_name(
        &mut self,
        screen: &mut a11y_vt100::Screen,
        icon_name: &[u8],
    ) {
        std::hint::black_box((screen, icon_name));
    }

    fn set_window_title(&mut self, screen: &mut a11y_vt100::Screen, title: &[u8]) {
        std::hint::black_box((screen, title));
    }

    fn unhandled_char(&mut self, screen: &mut a11y_vt100::Screen, c: char) {
        std::hint::black_box((screen, c));
    }

    fn unhandled_control(&mut self, screen: &mut a11y_vt100::Screen, b: u8) {
...
fn process_frames(frames: &[Vec<u8>]) {
    let mut parser = a11y_vt100::Parser::new_with_callbacks(24, 80, 0, Callbacks);
    for frame in frames {

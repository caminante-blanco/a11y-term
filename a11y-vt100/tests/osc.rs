...
fn title_icon_name() {
    ...
    impl a11y_vt100::Callbacks for Window {
        fn set_window_icon_name(
            &mut self,
            _: &mut a11y_vt100::Screen,
            icon_name: &[u8],
        ) {
            self.icon_name =
                std::str::from_utf8(icon_name).unwrap().to_string();
        }
        fn set_window_title(&mut self, _: &mut a11y_vt100::Screen, title: &[u8]) {
            self.title = std::str::from_utf8(title).unwrap().to_string();
        }
    }

    let mut parser =
        a11y_vt100::Parser::new_with_callbacks(24, 80, 0, Window::default());
...
fn clipboard() {
    ...
    impl a11y_vt100::Callbacks for Clipboard {
        ...
        fn paste_from_clipboard(&mut self, _: &mut a11y_vt100::Screen, ty: &[u8]) {
            self.pasted.push(ty.to_vec());
        }

        fn unhandled_osc(&mut self, _: &mut a11y_vt100::Screen, params: &[&[u8]]) {
            panic!("unhandled osc: {params:?}");
        }
    }

    let mut parser =
        a11y_vt100::Parser::new_with_callbacks(24, 80, 0, Clipboard::default());
